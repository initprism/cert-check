            ...
            for certificate in certificates.iter() {
                ...
                if is_ca {
                    ...
                    for rdn_seq in &issuer.rdn_seq {
                        if let Some(attribute) = rdn_seq.set.get(0) {
                            match oid2sn(&attribute.attr_type) {
                                Ok(s) => {
                                    if let Ok(rdn_content) = attribute.attr_value.content.as_str() {
                                        if s == "CN" {
                                            intermediate_cert.issuer = rdn_content.to_string();
                                        }
                                    }
                                }
                                Err(_e) => return Err(Error::new(ErrorKind::Other, "Error converting Oid to Nid".to_string()))
                    ...
                    for rdn_seq in &subject.rdn_seq {
                        if let Some(attribute) = rdn_seq.set.get(0) {
                            match oid2sn(&attribute.attr_type) {
                                Ok(s) => {
                                    if let Ok(rdn_content) = attribute.attr_value.content.as_str() {
                                        let rdn_content = rdn_content.to_string();
                                        match s {
...
mod tests {
    ...
    fn test_check_ssl_server_is_valid() {
        assert!(CertCheck::from_domain("rust-lang.org").unwrap().server.is_valid);
    }

    #[test]
    fn test_check_ssl_server_is_invalid() {
        let actual = CertCheck::from_domain("expired.badssl.com").map_err(|e| e.kind());
